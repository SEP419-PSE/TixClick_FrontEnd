...
import { ArrowLeft, Calendar, Loader2, LogIn, MapPin, Ticket } from "lucide-react"
...
import useTicketByEventId, { type TicketResponse } from "../hooks/useTicketByEventId"
...
import { formatDateVietnamese, formatMoney, formatTimeFe, parseSeatCode } from "../lib/utils"
        ...
        {section.type == "STANDING" ? (
          <div
            ...
            style={{
              ...
              border: selectedSeats.some((s) => s.id === `standing-${section.id}`) ? "3px solid #059669" : "none",
...
const TicketBooking = () => {
  ...
  const [loadingChangeTicket, setLoadingChangeTicket] = useState<boolean>(false)
  ...
  const validateSeatQuantity = (seats: any[], tickets: TicketResponse[]): boolean => {
    const seatCount: { [key: string]: number } = {}

    seats.forEach((seat) => {
      if (seat.quantity) {
        // Nếu có quantity, cộng thêm vào số ghế
        seatCount[seat.seatTypeId] = (seatCount[seat.seatTypeId] || 0) + seat.quantity
      } else {
    ...
    for (const seatTypeId in seatCount) {
      ...
      if (ticket && seatCount[seatTypeId] > ticket.maxQuantity) {
        toast.error(`Số vé được mua tối đa của ghế ${ticket.name} là ${ticket.maxQuantity}`)
  ...
  const handleSeatClick = (seat: ISeat, sectionName: string, quantityAction?: "increase" | "decrease") => {
    ...
    if (isStandingSection) {
      ...
      if (isSelected && !quantityAction) {
        ...
        const zoneId = section.zoneActivityId || Number.parseInt(section.id || "0")
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      console.log("Ticket purchase requests:", JSON.stringify(ticketPurchaseRequests, null, 2))
  ...
  const handlechangeTicket = async () => {
    ...
    const totalQuantity = ticketPurchaseRequests.reduce((sum, req) => sum + (req.quantity || 0), 0)
        ...
        <div className="fixed flex flex-col top-0 right-0 h-full w-80 pt-[70px] bg-white shadow-md border border-gray-200">
                ...
                >
                  ...
                  <div>
                    <div className="flex items-center font-medium text-gray-900">
                      <p>
                        {seatType.name}{" "}
                        <span className="text-xs text-pse-gray ml-auto">(Tối đa {seatType.maxQuantity} vé)</span>
                      </p>
                    </div>
          ...
          <div className="bg-pse-black-light text-white p-4">
            <div className="mb-2 flex items-center text-xs gap-1">
              ...
              {selectedSeats.length > 0
                ? selectedSeats.map((seat) => `(${seat.sectionName} ${parseSeatCode(seat.rcCode)})`).join(", ")
                : "Chưa chọn ghế"}
            </div>

            <Button
              className={`w-full ${
                selectedSeats.length != 0 ? "bg-pse-green text-white" : "bg-white text-pse-gray"
              }  font-semibold hover:bg-opacity-70 transition-all duration-300`}
              disabled={selectedSeats.length === 0 || isLoading || loadingChangeTicket}
              onClick={changeTicket == true ? handlechangeTicket : handleProceedToPayment}
            >
              {isLoading ? (
                <>
